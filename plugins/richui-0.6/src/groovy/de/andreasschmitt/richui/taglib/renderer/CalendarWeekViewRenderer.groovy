		...
		def getEvents = { Calendar day, int hour, int minute ->
			...
			attrs?.items.each { item ->
				...
				attrs?.constraintDateFields?.each { constraintDateField ->
					...
					else if(item?."${constraintDateField}".month == itemDate.month && item?."${constraintDateField}".year == itemDate.year && item?."${constraintDateField}".date == itemDate.date && item?."${constraintDateField}".hours == hour && determineMinutes(item."${constraintDateField}".minutes) == minute){
						events.add(item)
					}
				}
			}
